...
def getPolicy(_policy_name: str):
	...
	for _policy in _all_policies["policies"]:
		if _policy_name == _policy["name"]:
			return _policy
	return None

# Get All Available Host Fields
def getHostFields():
	_header						= makeHeaderWebApi()
...
def getHostLastSeen(ip: str) -> str:
	return getHost(ip)["online"]["timestamp"]
...
def getSegments():
	...
	def cidr(_range):
		if "-" in _range:
			...
			_end	    			= ipaddress.IPv4Address(_range[1])
